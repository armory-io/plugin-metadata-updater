...
class MetadataRepoUpdater : CliktCommand() {
    ...
    private val tempDir =
        Files.createTempDirectory("plugin-metadata-").also {
    ...
    override fun run() {
        ...
        val metadataRepo =
            "plugins.json".resolve().inputStream().use {
        ...
        git
            .commit()
            .setMessage("Release ${metadata.id} ${metadata.releases.first().version}")
            .setAll(true)
            .call()
        ...
        val pr =
            githubRepo.createPullRequest(
                // title=
                "Release ${metadata.id} ${metadata.releases.first().version}",
                // head=
                "$owner:$branch",
                // base=
                "master",
...
internal fun PluginMetadata.clean() =
    copy(
        releases =
            releases
                .map {
                    it.copy(
                        version =
                            if (it.version.startsWith("v")) {
                                it.version.substring(1)
                            } else {
                                it.version
                            },
                    )
...
internal fun PluginMetadata.withURL(url: String) =
    copy(
